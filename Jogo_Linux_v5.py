...
def strelevante(string):##Salva as strings relevantes, para que elas continuem na tela e reapareçam após o jogo ser fechado e reaberto.
    f1 = open('main.sav','r')    
    f2 = open('tmp.sav','w')
    for i in f1:
        if i.startswith("It"):
...
def printrelevante():#Printa na tela os textos relevantes, para diminuir a poluição visual 
    f1 = open('main.sav','r')
    for i in f1:
        if i.startswith("It"):
            break
...
def saveitens(itens):
    dummy = '' #Variavel pra montar uma string.
    f1 = open('main.sav','r')
    f2 = open('tmp.sav','w')
    for i in f1:
        f2.write(i)
    ...
    for k in itens:
        if k!= itens[-1]:
            ...
            dummy += k
    f2.write(dummy + "\n")
    dummy = 0 # a mesma dummy agr sendo usada pra saltar a primeira linha
    #f2.write(i)
    for i in f1:
        if dummy != 0:
            f2.write(i)
...
def savegame(itens,magias,parametros):

    saveitens(itens)
    savemagics(magias)
    saveparameters(parametros)
...
def PuzzleElevador(itens, magias):
    strelevante("Você caminha ate alcançar uma colina muito alta, não parece ser possivel escalá-la")
    strelevante("Você esta sobre uma tábua que parece estar presa por uma corda passando por uma roldana no teto")
...
def tutorial(): #tutorial do jogo.
    ...
    return itens,magias,parametros

if __name__ == '__main__':
    print("ATENÇÃO !!!!!!!!")
    time.sleep(2)
    print("Para jogar esse jogo é necessário ter um arquivo de save do jogo")
    time.sleep(2)
    print("Caso essa seja sua primeira vez jogando, recomendo jogar o tutorial")
    time.sleep(2)
    print("Jogar o tutorial automaticamente criará o arquivo de save.")
    userInput = str(input("Deseja jogar o tutorial?(sim/nao) "))
    if userInput.startswith("s"):
        os.system('cls')
        criasave()
        itens, magias, parametros = tutorial()
        savegame(itens,magias,parametros)
        time.sleep(10)
    else:
        userInput = str(input("Deseja começar um novo jogo?(sim/nao) "))
        if userInput.startswith("s"):
            criasave()
        print("Essa parte do jogo ainda está em desenvolvimento meu chapinha")
        time.sleep(10)